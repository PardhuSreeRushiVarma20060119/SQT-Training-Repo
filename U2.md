...
| **Expected Behavior** | Tests normal, intended system usage |
| **Requirement Validation** | Directly verifies stated functional requirements |
| **User Perspective** | Follows typical user workflows |
| **Success Oriented** | Expects system to perform functions successfully |
| **Business Logic** | Validates core business rules and processes |

---

### 6.3 Purpose and Importance

**Why Positive Testing is Critical:**

1. **Requirement Verification:** Confirms system implements specified features
2. **Core Functionality:** Validates essential business operations
3. **User Satisfaction:** Ensures typical use cases work smoothly
4. **Baseline Establishment:** Creates foundation for other test types
5. **Acceptance Criteria:** Forms basis for user acceptance testing
6. **Demonstration Value:** Shows stakeholders the system works as intended

**When to Focus on Positive Testing:**
...
### 6.4 Relationship to Requirements

**Direct Mapping:**
```
Requirement: "System shall allow users to login with username and password"
         ↓
Positive Test Case: "Verify successful login with valid username and valid password"
```

**Coverage Goal:** Every functional requirement should have at least one positive test case.

---

### 6.5 Comprehensive Examples

#### **Example 1: E-Banking Login (Positive Cases)**

**Requirement:** REQ-AUTH-101: System shall authenticate users with valid credentials

| **Test Case ID** | **Test Case Description** | **Test Data** | **Expected Result** |
|------------------|---------------------------|---------------|---------------------|
| **TC-AUTH-001** | Verify login with valid username and password | Username: testuser01<br>Password: Test@123 | Login successful, redirected to Dashboard |
| **TC-AUTH-002** | Verify login with valid email and password | Email: user@example.com<br>Password: Test@123 | Login successful, redirected to Dashboard |
| **TC-AUTH-003** | Verify login preserves username case insensitivity | Username: TESTUSER01<br>Password: Test@123 | Login successful (system treats username as case-insensitive) |
| **TC-AUTH-004** | Verify "Remember Me" functionality | Username: testuser01<br>Password: Test@123<br>Remember Me: Checked | Login successful, session persists beyond 15 minutes |
| **TC-AUTH-005** | Verify successful login updates last login timestamp | Username: testuser01<br>Password: Test@123 | Login successful, database `last_login` field updated |

---

#### **Example 2: E-Commerce Shopping Cart (Positive Cases)**

**Requirement:** REQ-CART-201: Users shall be able to add products to shopping cart

| **Test Case ID** | **Test Case Description** | **Test Data** | **Expected Result** |
|------------------|---------------------------|---------------|---------------------|
| **TC-CART-001** | Verify adding single product to empty cart | Product: Wireless Mouse<br>Quantity: 1 | Product added, cart shows 1 item, total = $29.99 |
| **TC-CART-002** | Verify adding multiple products to cart | Product 1: Wireless Mouse<br>Product 2: Keyboard<br>Quantity: 1 each | Cart shows 2 items, total = $29.99 + $49.99 = $79.98 |
| **TC-CART-003** | Verify updating product quantity in cart | Product: Wireless Mouse<br>Initial Quantity: 1<br>Update to: 3 | Quantity updated to 3, total = $89.97 |
| **TC-CART-004** | Verify removing product from cart | Product: Wireless Mouse<br>Action: Remove | Product removed, cart empty or shows other items |
| **TC-CART-005** | Verify applying valid discount code | Cart Total: $100<br>Discount Code: SAVE10<br>(10% off) | Discount applied, new total = $90.00 |
| **TC-CART-006** | Verify cart persists across sessions | Add product, logout, login again | Cart retains previously added products |

---

#### **Example 3: User Registration (Positive Cases)**

**Requirement:** REQ-REG-301: System shall allow new users to register with valid information

| **Test Case ID** | **Test Case Description** | **Test Data** | **Expected Result** |
|------------------|---------------------------|---------------|---------------------|
| **TC-REG-001** | Verify successful registration with all mandatory fields | Name: John Doe<br>Email: john@example.com<br>Password: Secure@123<br>Phone: +1-555-0100 | Registration successful, confirmation email sent, user can login |
| **TC-REG-002** | Verify registration with optional fields completed | All mandatory + Address, DOB | Registration successful with all data saved |
| **TC-REG-003** | Verify password complexity validation accepts valid password | Password: Complex@Pass1234 (12 chars, mixed case, number, special) | Password accepted, meets complexity rules |
| **TC-REG-004** | Verify email verification link activates account | Registration complete, click verification link in email | Account status changes to "Active", user can now login |
| **TC-REG-005** | Verify successful registration creates user profile | Complete registration | User record created in database, profile page accessible |

---

### 6.6 Positive Test Case Design Strategy

**Step 1: Identify Valid Input Domains**
- Determine all acceptable input ranges
- Consider different valid formats (email, phone)
- Include boundary values that are VALID

**Step 2: Map Happy Path Workflows**
- Trace typical user journeys
- Include successful end-to-end flows
...
**Step 4: Include Integration Points**
- API responses with valid data
- Database operations (CRUD)
- Third-party service integrations

**Example Positive Testing Coverage Checklist:**
```
For Login Feature:
☑ Valid username + valid password
☑ Alternate valid formats (email vs username)
☑ Case variations in username
☑ Special features (Remember Me, SSO)
...
**Negative Test Cases** validate that the system handles invalid inputs, unexpected conditions, and error scenarios gracefully. They verify the system's robustness, error handling, and ability to prevent incorrect operations.

**Objective:** Confirm that the system:
- Rejects invalid inputs appropriately
- Displays meaningful error messages
- Maintains data integrity under adverse conditions
- Prevents unauthorized or unsafe operations
- Fails safely without crashing or data corruption

**Fundamental Principle:** Negative testing answers "Does the system properly handle when things go wrong?"
...
| Characteristic | Description |
|---------------|-------------|
| **Invalid Input Focus** | Uses illegitimate, unacceptable, or malicious input values |
| **Error Handling** | Tests system response to erroneous conditions |
| **Boundary Violations** | Tests values outside valid ranges |
| **Security Validation** | Tests injection attacks, unauthorized access |
| **Robustness** | Confirms system doesn't crash or behave unpredictably |
| **User Protection** | Prevents users from making mistakes |

---

### 7.3 Purpose and Importance

**Why Negative Testing is Critical:**

1. **Security:** Prevents vulnerabilities (SQL injection, XSS, authentication bypass)
2. **Data Integrity:** Ensures invalid data cannot corrupt the database
3. **User Experience:** Provides helpful error messages, prevents frustration
4. **System Stability:** Prevents crashes and unexpected behavior
5. **Compliance:** Validates input validation per regulatory requirements
6. **Risk Mitigation:** Reduces production incidents and support costs

**Industry Statistics:**
- 70-80% of security vulnerabilities stem from inadequate input validation
- Negative testing often reveals more critical defects than positive testing
- Production failures frequently occur due to unhandled edge cases

---

### 7.4 Categories of Negative Testing

#### **1. Invalid Input Testing**
Testing with data that violates format, type, or business rules

**Examples:**
- Alphabetic characters in numeric fields
- Invalid email formats
- Passwords not meeting complexity rules
- Negative quantities
...
**Examples:**
- Password length: 7 characters (when minimum is 8)
- Age: -1 or 151 years
- Quantity: 0 or 10,001 (when max is 10,000)

#### **3. Missing Mandatory Data**
Testing with required fields left blank

**Examples:**
- Submit form without username
- Checkout without shipping address
- API call without required parameters

#### **4. Authorization and Authentication Failures**
Testing unauthorized access attempts

**Examples:**
- Access protected page without login
- User attempting admin functions
- Expired session tokens

#### **5. Security Testing (Negative Approach)**
...
#### **6. State/Sequence Violations**
Testing actions in wrong order or state

**Examples:**
- Checkout before adding items to cart
- Submit payment before completing shipping info
...
**Examples:**
- Uploading file exceeding size limit
- Creating more items than maximum allowed
...
#### **Example 1: E-Banking Login (Negative Cases)**

| **Test Case ID** | **Test Case Description** | **Test Data** | **Expected Result** |
|------------------|---------------------------|---------------|---------------------|
| **TC-AUTH-010** | Verify login fails with blank username | Username: (empty)<br>Password: Test@123 | Error: "Username is required"<br>Login fails |
| **TC-AUTH-011** | Verify login fails with blank password | Username: testuser01<br>Password: (empty) | Error: "Password is required"<br>Login fails |
| **TC-AUTH-012** | Verify login fails with both fields blank | Username: (empty)<br>Password: (empty) | Error: "Username and Password are required"<br>Login fails |
| **TC-AUTH-013** | Verify login fails with invalid username | Username: nonexistentuser<br>Password: Test@123 | Error: "Invalid username or password"<br>Login fails, no user info leaked |
| **TC-AUTH-014** | Verify login fails with invalid password | Username: testuser01<br>Password: WrongPassword | Error: "Invalid username or password"<br>Login fails, failed attempt logged |
| **TC-AUTH-015** | Verify login fails with SQL injection in username | Username: `' OR '1'='1`<br>Password: anything | Error: "Invalid credentials"<br>SQL injection prevented |
| **TC-AUTH-016** | Verify login fails with XSS attempt in username | Username: `<script>alert('XSS')</script>`<br>Password: Test@123 | Input sanitized, script not executed, login fails |
| **TC-AUTH-017** | Verify login with excessively long username | Username: 1000-character string<br>Password: Test@123 | Error: "Username too long" or input truncated, login fails gracefully |
| **TC-AUTH-018** | Verify login with special characters only in password | Username: testuser01<br>Password: @@@@@@@ | Error: "Invalid credentials"<br>No system error |
| **TC-AUTH-019** | Verify account lockout after 3 failed attempts | Username: testuser01<br>Password: Wrong1, Wrong2, Wrong3 | After 3rd attempt: "Account locked due to multiple failed attempts" |
| **TC-AUTH-020** | Verify login with locked account using correct password | Username: lockeduser<br>Password: Correct@123 | Error: "Account is locked. Contact support."<br>Login denied even with correct credentials |

---

#### **Example 2: E-Commerce Shopping Cart (Negative Cases)**

| **Test Case ID** | **Test Case Description** | **Test Data** | **Expected Result** |
|------------------|---------------------------|---------------|---------------------|
| **TC-CART-015** | Verify adding out-of-stock product to cart | Product: Out-of-Stock Item<br>Stock: 0 | Error: "Product out of stock"<br>Cannot add to cart |
| **TC-CART-016** | Verify adding quantity exceeding available stock | Product: Limited Stock Item<br>Available: 5<br>Attempt: 10 | Error: "Only 5 items available"<br>Suggests adding max available |
| **TC-CART-017** | Verify adding negative quantity | Product: Any<br>Quantity: -5 | Error: "Quantity must be positive"<br>Addition fails |
| **TC-CART-018** | Verify adding zero quantity | Product: Any<br>Quantity: 0 | Error: "Quantity must be at least 1"<br>Addition fails |
| **TC-CART-019** | Verify applying invalid discount code | Cart Total: $100<br>Code: INVALID123 | Error: "Invalid discount code"<br>Discount not applied |
| **TC-CART-020** | Verify applying expired discount code | Cart Total: $100<br>Code: EXPIRED10<br>(Validity: Expired) | Error: "Discount code has expired"<br>Discount not applied |
| **TC-CART-021** | Verify checkout with empty cart | Cart: Empty<br>Action: Proceed to Checkout | Error: "Cart is empty. Add items before checkout"<br>Checkout blocked |
| **TC-CART-022** | Verify updating cart quantity to exceed maximum | Product: Any<br>Update to: 10,001<br>(Max: 10,000) | Error: "Maximum quantity is 10,000"<br>Update rejected |
| **TC-CART-023** | Verify removing non-existent product from cart | Product ID: 99999<br>(Does not exist in cart) | Error: "Product not found in cart"<br>No system crash |

---

#### **Example 3: User Registration (Negative Cases)**

| **Test Case ID** | **Test Case Description** | **Test Data** | **Expected Result** |
|------------------|---------------------------|---------------|---------------------|
| **TC-REG-010** | Verify registration fails with existing email | Email: existing@example.com<br>(Already registered) | Error: "Email already registered. Please login or use different email." |
| **TC-REG-011** | Verify registration fails with invalid email format | Email: invalidemail.com<br>(Missing @) | Error: "Invalid email format"<br>Registration fails |
| **TC-REG-012** | Verify registration fails with weak password | Password: 123<br>(Too short, no complexity) | Error: "Password must be 8+ characters with uppercase, lowercase, number, special character" |
| **TC-REG-013** | Verify registration fails with password = username | Username: johndoe<br>Password: johndoe | Error: "Password cannot be same as username"<br>Registration fails |
| **TC-REG-014** | Verify registration fails with mismatched password confirmation | Password: Secure@123<br>Confirm: Secure@456 | Error: "Passwords do not match"<br>Registration fails |
| **TC-REG-015** | Verify registration fails with invalid phone format | Phone: 123 (invalid) | Error: "Invalid phone number format. Use +1-XXX-XXX-XXXX" |
| **TC-REG-016** | Verify registration fails with future date of birth | DOB: 2030-01-01<br>(Future date) | Error: "Date of birth cannot be in future"<br>Registration fails |
| **TC-REG-017** | Verify registration fails with age < 18 | DOB: 2015-01-01<br>(Calculates to 11 years old) | Error: "You must be 18 or older to register" |
| **TC-REG-018** | Verify registration fails without accepting Terms | All fields valid<br>Terms checkbox: Unchecked | Error: "You must accept Terms and Conditions"<br>Registration blocked |
| **TC-REG-019** | Verify registration fails with XSS in name field | Name: `<script>alert('XSS')</script>` | Input sanitized, script tags removed/escaped, registration may proceed with sanitized input OR error shown |
| **TC-REG-020** | Verify registration fails with SQL injection in email | Email: `admin'--@example.com` | Input sanitized, SQL injection prevented, registration fails with format error |

---

### 7.6 Error Message Quality Criteria

**Effective Error Messages Must Be:**

1. **Clear and Specific**
   - ✗ "Error occurred"
   - ✓ "Password must be at least 8 characters"

2. **User-Friendly (Non-Technical)**
   - ✗ "NullPointerException in AuthService.java:line 45"
...
4. **Secure (No Information Leakage)**
   ...
   - ✓ "Invalid username or password" (ambiguous, doesn't reveal which is wrong)

5. **Consistent in Tone and Format**
   - All errors follow same style/location/color scheme

6. **Visible and Noticeable**
   - Highlighted in red, near relevant field
   - Not hidden in logs or developer console

---

### 7.7 Negative Test Case Design Strategy

**Step 1: Identify Invalid Input Domains**
- For each valid input, define invalid variations
- Consider type, format, length, range violations

**Step 2: Test Boundary Violations**
- Just below minimum
- Just above maximum
- Exactly at exclusion points

**Step 3: Test Missing Data**
- Omit each mandatory field individually
- Omit multiple fields
- Submit empty form

**Step 4: Test Security Vulnerabilities**
- OWASP Top 10 attack vectors
- SQL injection, XSS, CSRF
- Authentication and authorization bypass attempts

**Step 5: Test State Violations**
- Perform actions out of sequence
- Access resources in invalid state
...
**Example Negative Testing Coverage Checklist:**
```
For Login Feature:
☑ Empty username
☑ Empty password
...
## 8️⃣ SCENARIO vs TEST CASE — COMPREHENSIVE COMPARISON

### 8.1 Detailed Comparison Table

| **Aspect** | **Test Scenario** | **Test Case** |
|-----------|-------------------|---------------|
| **Definition** | High-level description of what to test | Detailed specification of how to test |
| **Level of Detail** | Abstract, conceptual | Concrete, specific |
| **Purpose** | Define testing scope and coverage | Execute actual testing |
| **Granularity** | Broad (one-to-many relationship) | Fine-grained (atomic) |
| **Executability** | Not directly executable | Directly executable |
| **Test Steps** | Not included | Detailed step-by-step instructions |
| **Test Data** | Not specified | Exact values specified |
| **Expected Results** | Not specified | Precisely defined |
| **Creation Phase** | Early (requirements/design phase) | Later (test design phase) |
| **Audience** | Business analysts, stakeholders, testers | Primarily testers and automation engineers |
| **Format** | Usually a single statement | Structured document with multiple fields |
| **Traceability** | Links to requirements | Links to scenarios AND requirements |
| **Review Effort** | Quick review (1-2 minutes per scenario) | Detailed review (5-15 minutes per test case) |
| **Maintenance** | Relatively stable | Changes frequently with UI/workflow changes |
| **Example Count** | 10-20 scenarios for a module | 100-200 test cases for a module |
| **Documentation Level** | Minimal | Comprehensive |
| **Usage** | Planning, coverage analysis | Execution, defect reporting, regression |

---

### 8.2 Hierarchical Relationship
...
├── TEST SCENARIO (TS-AUTH-001): "Verify user login with valid credentials"
│   │
│   ├── TEST CASE (TC-AUTH-001): Login with valid username and password
│   ├── TEST CASE (TC-AUTH-002): Login with valid email and password
│   ├── TEST CASE (TC-AUTH-003): Login with case-insensitive username
│   ├── TEST CASE (TC-AUTH-004): Login with Remember Me checked
│   └── TEST CASE (TC-AUTH-005): Login and verify database update
│
├── TEST SCENARIO (TS-AUTH-002): "Verify user login with invalid credentials"
│   │
│   ├── TEST CASE (TC-AUTH-010): Login with blank username
│   ├── TEST CASE (TC-AUTH-011): Login with blank password
│   ├── TEST CASE (TC-AUTH-012): Login with both blank
│   ├── TEST CASE (TC-AUTH-013): Login with invalid username
│   └── TEST CASE (TC-AUTH-014): Login with invalid password
│
└── TEST SCENARIO (TS-AUTH-003): "Verify account lockout mechanism"
    │
    ├── TEST CASE (TC-AUTH-019): Lockout after exactly 3 failed attempts
    ├── TEST CASE (TC-AUTH-020): Login with locked account
    └── TEST CASE (TC-AUTH-021): Account unlock after timeout
```
...
**REQUIREMENT:**
```
REQ-CART-201: Users shall be able to add products to shopping cart with quantity selection.
```
...
**TEST SCENARIO:**
```
Scenario ID: TS-CART-001
Scenario: Verify adding products to shopping cart
Requirement: REQ-CART-201
Priority: Critical
```

**That's it. The scenario stops here. No steps, no data, no expected results.**
...
Title: Verify adding single product to empty cart with quantity 1
Scenario ID: TS-CART-001
Requirement ID: REQ-CART-201

Preconditions:
- User logged in
- Cart is empty
- Product "Wireless Mouse" exists with price $29.99 and stock > 10

Test Steps:
1. Navigate to product page: /products/wireless-mouse
2. Verify "Add to Cart" button is visible
3. Verify quantity field shows default value "1"
4. Click "Add to Cart" button
5. Wait for confirmation message
6. Navigate to cart page: /cart

Test Data:
- Product: Wireless Mouse (ID: PROD-001)
- Quantity: 1
- Price: $29.99

Expected Results:
- Confirmation message: "Product added to cart"
- Cart page shows:
  * Product Name: Wireless Mouse
  * Quantity: 1
  * Unit Price: $29.99
  * Subtotal: $29.99
  * Cart item count badge: 1
- Database: cart_items table has 1 record for user with product_id=PROD-001
...
Title: Verify adding product with quantity 5
Scenario ID: TS-CART-001
Requirement ID: REQ-CART-201
...
- User logged in
- Cart is empty
- Product "Keyboard" exists with stock >= 5

Test Steps:
1. Navigate to product page: /products/keyboard
2. Locate quantity input field
3. Clear default value
4. Enter quantity: 5
5. Click "Add to Cart" button
6. Navigate to cart page

Test Data:
- Product: Keyboard (ID: PROD-002)
- Quantity: 5
- Price: $49.99
...
- Database: cart_items.quantity = 5
...
- Conducting initial test planning
- Reviewing coverage with stakeholders
- Estimating testing effort
- Creating high-level test strategy
- Communicating with non-technical audiences
- Checking requirement-to-test traceability
- Test scenarios are sufficient for early project phases

**Use Test Cases When:**
- Ready for actual test execution
- Need precise defect reproduction steps
...
### 8.5 Conversion Process: Scenario → Test Cases

**Step-by-Step Derivation:**

1. **Start with Scenario:** TS-AUTH-001: "Verify user login with valid credentials"

2. **Identify Variations:**
   - Different valid credential types (username vs email)
   - Different input formats (case variations)
   - Different features (Remember Me)
   - Different verification points (UI, database, logs)

3. **Create Atomic Test Cases:**
   - TC-001: Basic valid login
   - TC-002: Login with email instead of username
   - TC-003: Login with uppercase username
   - TC-004: Login with Remember Me
   - TC-005: Verify database update

4. **Add Detailed Specifications** to each test case:
   - Preconditions
   - Step-by-step instructions
   - Exact test data
   - Measurable expected results

5. **Link Back to Scenario and Requirement**

**Result:** 1 Scenario → 5-10 Test Cases (typical expansion ratio)

---

## ✅ SUMMARY AND KEY TAKEAWAYS

### Unit-2 Core Concepts Recap

1. **Software Testing** is a systematic discipline for verification and validation, serving as a risk-reduction mechanism and quality gate in SDLC.

2. **Test Plan (IEEE 829)** is the strategic blueprint for testing, containing 16 mandatory components that ensure comprehensive planning, resource allocation, and stakeholder alignment.

3. **Test Planning Process** follows 6 structured steps: Requirement Analysis → Scope Definition → Risk Identification → Strategy Selection → Resource Planning → Review & Approval.

4. **Test Scenarios** are high-level, user-centric descriptions of WHAT to test, serving as a bridge between requirements and executable test cases.

5. **Test Cases** are detailed, step-by-step executable specifications of HOW to test, containing preconditions, test steps, test data, expected results, and actual results.

6. **Positive Test Cases** validate expected behavior with valid inputs, confirming the system does what it's supposed to do.

7. **Negative Test Cases** validate error handling and robustness with invalid inputs, confirming the system properly handles what should NOT happen.
...
|---------------|-------------------|
| Testing vs QA | Testing detects defects (product); QA prevents defects (process) |
| Testing vs Debugging | Testing finds defects; Debugging fixes defects |
| Test Plan vs Test Case | Plan is strategy; Case is execution instruction |
| Test Scenario vs Test Case | Scenario is WHAT (abstract); Test Case is HOW (concrete) |
| Positive vs Negative | Positive = valid input, expected success; Negative = invalid input, expected failure |
...
**Can you explain:**
- ☐ All 16 components of IEEE 829 Test Plan with examples?
- ☐ The 6-step test planning process?
- ☐ Difference between test scenario and test case with examples?
- ☐ All mandatory fields in a test case?
- ☐ Purpose and examples of positive test cases?
- ☐ Purpose and examples of negative test cases?

**Can you create:**
- ☐ A test plan section (e.g., Scope, Strategy, Risks)?
- ☐ Test scenarios for a given requirement?
- ☐ Detailed test cases from test scenarios?
- ☐ Both positive and negative test cases for any feature?

**Can you analyze:**
- ☐ Whether a test plan is complete per IEEE 829?
- ☐ Whether a test case is well-written and executable?
- ☐ Coverage gaps in test scenarios?

---

### Professional Quality Standards

**A Gold-Standard Test Case Must:**
1. Be executable by any qualified tester without ambiguity
2. Have one clear pass/fail outcome
3. Trace to requirements and scenarios
4. Include all mandatory fields
5. Use precise, measurable expected results
6. Be independent and repeatable
7. Follow organizational standards

**A Gold-Standard Test Plan Must:**
1. Include all 16 IEEE 829 components
2. Be approved by all stakeholders
3. Have measurable entry/exit criteria
4. Identify and mitigate risks
5. Assign clear responsibilities
6. Provide realistic schedules
7. Define quality thresholds

---

**You are now equipped with comprehensive, exam-ready, and industry-grade knowledge of Unit-2: Software Testing. This material covers test planning per IEEE 829 standards, test scenario creation, test case design, and the critical distinctions between positive and negative testing—all foundational skills for a professional Software Quality Engineer.**
